...
export default function Characters({ characters }) {
  const [value, setValue] = useState(12); // on default mount, shows 12 characters
...
// This is explained from my other builds but yeah, the fetched database will be static(the meaning yeah the one with the non movement) on mount on the website
//  So no loading times
export async function getStaticProps(context) {
  const characters = await fetch(
    "https://akabab.github.io/starwars-api/api/all.json"
  ).then((res) => res.json());
  // ooohhh the .then resolves the api, kinda like the promise being extracted with async await smth like that
